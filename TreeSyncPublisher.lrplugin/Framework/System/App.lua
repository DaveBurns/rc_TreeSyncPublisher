...
function App:showBezel( opts, fmt, ... )
    opts = opts or {}
    local dur = opts.dur or 3
    local holdoff = opts.holdoff or 1.2 -- default holdoff in previous version was 3, which gave time to do something - e.g. reload plugin, after killing an error box.. - if such cases present, just add a delay upon return.
    if LrTasks.canYield() then
...
function App:typeAssert( var, name, expType, m, ... )
    ...
    elseif type( var ) == Type( expType ) then
        return var
    else
